...
void MCP2515::transmitSPI(uint8_t data) {
    HAL_SPI_Transmit(&hspi1, &data, 1, 10);
}

void MCP2515::receiveSPI(uint8_t *data) {
    HAL_SPI_Receive(&hspi1, data, 1, 10);
}

MCP2515::MCP2515(SPI_HandleTypeDef* _SPI) : SPIHandle(_SPI)
{}

MCP2515::ERROR MCP2515::reset(void)
{
    startSPI();
    transmitSPI(INSTRUCTION_RESET);
    endSPI();
...
void MCP2515::readRegisters(const REGISTER reg, uint8_t values[], const uint8_t n)
{
    ...
    // mcp2515 has auto-increment of address-pointer, read back to back
...
{
    startSPI();
    transmitSPI(INSTRUCTION_BITMOD);
    transmitSPI(reg);
    transmitSPI(mask);
    transmitSPI(data);
    endSPI();
...
{
    startSPI();
    transmitSPI(INSTRUCTION_READ_STATUS);
    uint8_t status;
    receiveSPI(&status);
    endSPI();
